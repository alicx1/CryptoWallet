...
state = {
  ...
  updateCryptosFiltered(model) {
    ...
    const cFiltered = Object.values(this.data.cryptos.list).filter(function(obj){

      return (obj.code.toLowerCase().includes(filters.text.toLowerCase()) || obj.name.toLowerCase().includes(filters.text.toLowerCase())) && obj.price >= filters.price;
    });
  ...
  updateFiatsFiltered(model) {
    const filters = model.ui.currenciesCard.tabs.fiats.filters.text;
    const fFiltered = Object.values(this.data.fiats.list).filter(function(obj){

      return obj.name.toLowerCase().includes(filters.toLowerCase()) || obj.code.toLowerCase().includes(filters.toLowerCase());
    });
  ...
  updateSortOrder(model, currency) {
    ...
    this.data[currency].filtered.sort( (a,b) => {
      switch (typeof(a[prop])) {
        case 'number': return order*(a[prop] - b[prop]);
        case 'string': return order*(a[prop].localeCompare(b[prop]));
      }
    });
